 ...
 * coordinator, so a grouped room has to read them from the room it follows.
 */
...
const TABS: { id: LibraryTab; label: string; icon: React.JSX.Element }[] = [
    ...
    { id: 'recent', label: 'recent', icon: <History fontSize="small" /> },
...
/**
 * Favorites, Sonos playlists, the queue, recently played tracks and the browsable sources of the
 * speaker (radio, music library, line-in, TV and the music services).
 */
export default class SourceBrowser extends React.Component<SourceBrowserProps, SourceBrowserState> {
    constructor(props: SourceBrowserProps) {
        super(props);
        this.state = { tab: 'favorites', query: '', path: [] };
    }
    ...
    private browseResult(): MediaBrowseResult | null {
        return this.json<MediaBrowseResult>(this.props.coordinator, 'media_browse_result');
    }
    ...
    private queue(): QueueTrack[] {
        const array = this.json<QueueTrack[]>(this.props.coordinator, 'queue_array');
        if (array?.length) {
            return array.map(track => ({ ...track, title: track.title || '' }));
        }

        const raw = this.props.getValue(this.props.coordinator, 'queue');
        return String(raw ?? '')
            .split(/\r?\n|, /)
            .map(line => line.trim())
            .filter(Boolean)
    ...
    render(): React.JSX.Element {
        ...
        if (tab === 'favorites') {
            list = (this.json<string[]>(coordinator, 'favorites_list_array') || [])
                .filter(matches)
                .map(name =>
            ...
            list = (this.json<string[]>(coordinator, 'playlist_list_array') || [])
                .filter(matches)
                .map(name =>
            ...
            list = (this.json<RecentTrack[]>(ip, 'recent_tracks') || [])
        ...
        return (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, flex: 1, minHeight: 0 }}>
                <ToggleButtonGroup
                    ...
                    value={tab}
                    sx={{ flexWrap: 'wrap' }}
                    onChange={(_e, value: LibraryTab | null) => value && this.setTab(value)}
                >
                    {TABS.map(entry => (
                        <ToggleButton
                            key={entry.id}
